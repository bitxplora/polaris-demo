...
export default class Password extends Component<PasswordSignature> {
  ...
  <template>
    ...
    <div class="relative">
      <input
        class="w-64 h-8 rounded pl-2 border border-purple-950 sm:w-80"
        type={{if this.eyeToggled "text" "password"}}
        id="password"
        name="password"
        required
      />
      <div
        class="absolute size-2 border-0 top-1 left-56 sm:left-72"
        role="button"
        tabindex="0"
        aria-label="ToggleButton"
        {{on "click" this.toggleEye}}
      >
        {{#if this.eyeToggled}}
          <svg
