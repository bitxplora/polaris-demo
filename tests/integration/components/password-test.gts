...
module('Integration | Component | password', function (hooks) {
  ...
  test('it renders password component', async function (assert) {
    ...
    assert
      .dom('div>svg')
      .exists('The password vissibility toggle button exists.');
    assert
      .dom('div>svg')
      .hasClass('lucide-eye-off', 'The SVG has a class of "lucide-eye-off"');

    assert
      .dom('input')
      .hasClass(
        'w-64',
        'The width of the input tag is 16rem on a very small mobile screen'
      );
    assert
      .dom('input~div')
      .hasClass(
        'left-56',
        'The toggle button is positioned to right edge of input tag.'
      );

    await click('.lucide-eye-off');
    assert
      .dom('div>svg')
      .hasClass('lucide-eye', 'The SVG has a class of "lucide-eye"');
    ...
    assert
      ...
      .hasClass(
        'sm:w-80',
        'The width of the input tag is 20rem on a big screen'
      );
    assert
      .dom('input~div')
      .hasClass(
